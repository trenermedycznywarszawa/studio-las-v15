...
At every observable moment, there is at most one current authoritative release. Whenever the client is expected to act, there is exactly one. Zero current releases are allowed only before first publication; after an explicit pause, invalidation, referral, or closure; or after an exact hard validity boundary expires, including an exact review point that Damian explicitly designated in advance as that hard boundary. Expiry enforces a prior trainer-authored boundary; it is not a system-inferred pause or adaptation decision. In every zero-current state, the client-safe projection must not imply that any guidance remains actionable. Preparing v2 does not change v1. Activating v2 must retire v1 as part of one controlled replacement transition; if the transition cannot complete, v2 does not become current.
...
A `next review point` and a hard `validity boundary` are distinct unless Damian explicitly records the same exact point as both before activation. Reaching a next review point creates a review-due condition only: it must be surfaced to Damian, but it does not change focus, actionability, release authority, or publication state. The exact current release remains authoritative while its independent validity, entry, focus, stop, and safety conditions still hold. The event never selects progress, regression, continuation, pause, or closure. If Damian explicitly continues or replaces guidance, that decision records the next validity boundary or review point; pause, referral, and closure follow their own transition rules. If a hard validity boundary expires — including a review point explicitly designated as that boundary — the affected item and release become non-actionable and client-safe projections show no current instruction until Damian completes an explicit valid continuation or replacement.
...
| Declared validity boundary or exact next review point is missing, conflicting, or already passed before activation | Block activation; do not infer or silently extend a boundary |
| Hard validity boundary expires, including a review point explicitly designated as that boundary | Make the affected item and release non-actionable; allow zero current guidance; do not auto-progress or infer a pause decision |
| Next review point passes while guidance remains otherwise valid | Mark review due and surface it to Damian; keep the exact current release authoritative; do not change guidance automatically |
...
| Review anchor reached without a hard validity expiry | Mark review due only; keep the exact current release unchanged; no automatic pause, progression, summary, renewal, or closure |
...
| `focus-validity-boundary` | Every release/item names the exact current focus and client-safe purpose plus a validity boundary or exact next review point; hard expiry blocks action, while a passed review point creates review due and leaves otherwise-valid current guidance unchanged unless Damian designated it as the hard boundary | Guidance activates against another focus, remains actionable after hard expiry, or is auto-paused or changed by a review point |
