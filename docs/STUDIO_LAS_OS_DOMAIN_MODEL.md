...
**Status 2026-09-07: SUBORDINATE domain reference.** Reuse existing entities under the [Master Spec](STUDIO_LAS_OS_MASTER_SPEC.md). Proposed mappings, event objects and views below are not a current schema inventory or migration authorization. Runtime fields/enums remain unchanged; provenance and client-safe projection contracts retain their detailed authority.
