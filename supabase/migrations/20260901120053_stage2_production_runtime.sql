-- Stage 2 production runtime: inquiry -> human call -> trainer decision ->
-- explicit conversion into the existing client/PWD boundary.
-- Public ingress is intentionally absent here. A synthetic staging-only fixture
-- proves the boundary without changing the production Formspree transport.
...
create table public.inquiries (
  ...
  constraint inquiries_source_channel_check check (source_channel in ('public_first_contact', 'staging_fixture')),
  constraint inquiries_source_version_check check (length(btrim(source_version)) between 1 and 64),
  constraint inquiries_form_version_check check (length(btrim(form_version)) between 1 and 64),
  constraint inquiries_source_request_key_check check (length(btrim(source_request_key)) between 12 and 128),
  constraint inquiries_submitted_name_check check (length(btrim(submitted_name)) between 1 and 120),
  constraint inquiries_submitted_phone_check check (length(btrim(submitted_phone)) between 5 and 32),
  constraint inquiries_submitted_email_check check (
    submitted_email is null
    or (length(btrim(submitted_email)) between 3 and 320
      and submitted_email = lower(btrim(submitted_email))
      and position('@' in submitted_email) > 1)
  ),
  constraint inquiries_contact_window_check check (
    preferred_contact_window in (
      '16:00–18:00', '18:00–20:00', '20:00–22:00',
      'Najpierw napisz SMS i ustalmy termin', 'Inny termin — ustalimy wiadomością'
    )
  ),
  constraint inquiries_broad_goal_check check (
    broad_goal in (
      'Swobodniejsze poruszanie się na co dzień', 'Powrót do aktywności lub sportu',
      'Siła i kondycja po przerwie', 'Większa pewność w ruchu',
      'Chcę najpierw porozmawiać'
    )
  ),
  constraint inquiries_person_words_check check (
    person_words is null or length(btrim(person_words)) between 1 and 280
  ),
  constraint inquiries_privacy_version_check check (length(btrim(privacy_notice_version)) between 1 and 64),
  constraint inquiries_status_check check (inquiry_status in ('open', 'converted', 'closed')),
  constraint inquiries_contact_status_check check (
    contact_status in ('pending', 'contacting', 'completed', 'unreachable')
  ),
  constraint inquiries_next_action_type_check check (
    next_action_type is null
    or next_action_type in ('contact_call', 'contact_message', 'arrange_pwd', 'follow_up', 'referral')
  ),
  constraint inquiries_next_action_pair_check check (next_action_type is not null or next_action_at is null),
  constraint inquiries_closed_state_check check (
  ...
  constraint inquiries_converted_state_check check (
    (inquiry_status = 'converted' and converted_client_id is not null
      and converted_at is not null and converted_by_profile_id is not null)
    or (inquiry_status <> 'converted' and converted_client_id is null
      and converted_at is null and converted_by_profile_id is null)
  ),
...
create table public.inquiry_decisions (
  ...
  supersedes_decision_id uuid references public.inquiry_decisions(id) on delete restrict,
  decision_status text not null default 'active',
  superseded_at timestamptz,
  created_at timestamptz not null default now(),
  constraint inquiry_decisions_version_check check (decision_version > 0),
  constraint inquiry_decisions_value_check check (
    decision in ('PWD', 'FOLLOW_UP', 'NOT_NOW', 'REFERRED', 'NOT_A_FIT', 'CLOSED_BY_PERSON')
  ),
  constraint inquiry_decisions_goal_check check (length(btrim(goal_in_person_words)) between 1 and 500),
  constraint inquiry_decisions_why_now_check check (why_now is null or length(btrim(why_now)) between 1 and 500),
  constraint inquiry_decisions_barrier_check check (length(btrim(current_barrier)) between 1 and 500),
  constraint inquiry_decisions_rationale_check check (length(btrim(rationale)) between 1 and 1000),
  constraint inquiry_decisions_boundary_check check (
    boundary_note is null or length(btrim(boundary_note)) between 1 and 500
  ),
  constraint inquiry_decisions_next_action_check check (
    next_action_type is null
    or next_action_type in ('contact_call', 'contact_message', 'arrange_pwd', 'follow_up', 'referral')
  ),
  constraint inquiry_decisions_next_action_pair_check check (next_action_type is not null or next_action_at is null),
  constraint inquiry_decisions_status_check check (decision_status in ('active', 'superseded')),
  constraint inquiry_decisions_superseded_state_check check (
    (decision_status = 'active' and superseded_at is null)
    or (decision_status = 'superseded' and superseded_at is not null)
  ),
  constraint inquiry_decisions_source_version_check check (length(btrim(evidence_source_version)) between 1 and 64),
  constraint inquiry_decisions_form_version_check check (length(btrim(evidence_form_version)) between 1 and 64),
...
create unique index inquiry_decisions_one_active_idx
  on public.inquiry_decisions(inquiry_id) where decision_status = 'active';
create index inquiries_owner_open_idx
  on public.inquiries(owner_trainer_id, created_at desc) where inquiry_status = 'open';
...
create trigger audit_sensitive_row_change after insert or update or delete on public.inquiries
for each row execute function public.audit_sensitive_row_change();
create trigger audit_sensitive_row_change after insert or update or delete on public.inquiry_decisions
for each row execute function public.audit_sensitive_row_change();
...
as $$
  select exists (
    ...
    where auth.uid() is not null and private.is_trainer()
      and i.id = p_inquiry_id and i.owner_trainer_id = private.current_profile_id()
  );
$$;
...
create policy inquiries_trainer_aal2_gate on public.inquiries as restrictive for all to authenticated
  using (private.trainer_mfa_satisfied()) with check (private.trainer_mfa_satisfied());
create policy inquiry_decisions_owner_select on public.inquiry_decisions for select to authenticated
  using (private.trainer_owns_inquiry(inquiry_id));
create policy inquiry_decisions_trainer_aal2_gate on public.inquiry_decisions as restrictive for all to authenticated
  using (private.trainer_mfa_satisfied()) with check (private.trainer_mfa_satisfied());

create or replace function public.set_inquiry_contact_state(
  p_inquiry_id uuid, p_contact_status text, p_next_action_type text default null,
  p_next_action_at timestamptz default null, p_close_inquiry boolean default false
)
...
begin
  ...
  set contact_status = p_contact_status, next_action_type = v_next_action_type,
  ...
  return jsonb_build_object('inquiryId', p_inquiry_id, 'contactStatus', p_contact_status,
    'inquiryStatus', case when p_close_inquiry then 'closed' else 'open' end);
end;
$$;

create or replace function public.save_inquiry_decision(
  p_inquiry_id uuid, p_decision text, p_goal_in_person_words text,
  p_why_now text, p_current_barrier text, p_rationale text,
  p_boundary_note text default null, p_next_action_type text default null,
...
begin
  ...
  if v_next_action_type is not null and v_next_action_type not in (
    'contact_call', 'contact_message', 'arrange_pwd', 'follow_up', 'referral'
  ) then raise exception 'invalid inquiry next action' using errcode = '22023'; end if;
  if v_next_action_type is null and p_next_action_at is not null then
    raise exception 'next action type required for due time' using errcode = '22023';
  end if;
  ...
  select * into v_previous from public.inquiry_decisions
  where inquiry_id = p_inquiry_id and decision_status = 'active' for update;
  v_version := coalesce(v_previous.decision_version, 0) + 1;
  if v_previous.id is not null then
    update public.inquiry_decisions set decision_status = 'superseded', superseded_at = now()
  ...
  update public.inquiries
  set contact_status = 'completed', next_action_type = v_next_action_type,
      next_action_at = p_next_action_at,
  ...
  where id = p_inquiry_id;
  return jsonb_build_object('decisionId', v_decision_id, 'decisionVersion', v_version,
...
begin
  ...
  if v_inquiry.id is null then raise exception 'inquiry not found' using errcode = 'P0002'; end if;
  if v_inquiry.inquiry_status = 'converted' then
    return jsonb_build_object('clientId', v_inquiry.converted_client_id,
      'inquiryId', v_inquiry.id, 'alreadyConverted', true);
  end if;
  ...
  select * into v_decision from public.inquiry_decisions
  where inquiry_id = p_inquiry_id and decision_status = 'active' for update;
  if v_decision.id is null or v_decision.decision <> 'PWD'
  ...
  insert into public.clients (
    ...
    v_actor_profile_id, btrim(v_inquiry.submitted_name),
    nullif(lower(btrim(coalesce(v_inquiry.submitted_email, ''))), ''),
    btrim(v_inquiry.submitted_phone), null, null, 'diagnostic_visit', 1, 'active'
  ) returning id into v_client_id;
  update public.inquiries
  set inquiry_status = 'converted', next_action_type = null, next_action_at = null,
      converted_client_id = v_client_id, converted_at = now(),
      converted_by_profile_id = v_actor_profile_id, closed_at = null, closed_by_profile_id = null
  where id = p_inquiry_id;
  return jsonb_build_object('clientId', v_client_id, 'inquiryId', p_inquiry_id,
    'alreadyConverted', false);
end;
$$;

revoke all on function public.set_inquiry_contact_state(uuid, text, text, timestamptz, boolean)
  from public, anon, authenticated;
revoke all on function public.save_inquiry_decision(uuid, text, text, text, text, text, text, text, timestamptz)
  from public, anon, authenticated;
revoke all on function public.convert_inquiry_to_pwd_client(uuid) from public, anon, authenticated;
grant execute on function public.set_inquiry_contact_state(uuid, text, text, timestamptz, boolean)
  to authenticated;
grant execute on function public.save_inquiry_decision(uuid, text, text, text, text, text, text, text, timestamptz)
  to authenticated;
grant execute on function public.convert_inquiry_to_pwd_client(uuid) to authenticated;

comment on table public.inquiries is
  'Pre-client first-contact episodes only. No health intake, scoring, PWD, Guidance, account, or client-safe content.';
comment on table public.inquiry_decisions is
  'Versioned trainer-owned inquiry decisions. Protected mutations occur only through validated AAL2 RPCs.';
comment on column public.inquiries.retention_review_at is
  'Nullable staging field only until a separate privacy decision defines production retention.';
comment on function public.convert_inquiry_to_pwd_client(uuid) is
  'Atomically creates exactly one existing client from allowlisted name/email/phone fields after an owner AAL2 PWD decision. No PWD, Guidance, account, or publication side effects.';
