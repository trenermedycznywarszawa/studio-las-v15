...
-- This helper may bypass one user trigger only for exact synthetic pre-PWD QA data.
...
begin
  ...
  if v_client_id is not null then
    ...
    -- The production integrity trigger correctly freezes non-empty source evidence.
    -- This staging-only SECURITY DEFINER helper disables only that named trigger,
    -- after exact QA guards, and reenables it before any other cleanup work.
    execute 'alter table public.client_intakes disable trigger imported_intake_source_guard';
    ...
    execute 'alter table public.client_intakes enable trigger imported_intake_source_guard';
  end if;
