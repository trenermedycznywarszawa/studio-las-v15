...
const stripSqlComments = source => source.replace(/--[^\n]*/g, "");
...
assert.match(executableSql, /prepare_questionnaire_e2e_db/);
assert.match(executableSql, /cleanup_questionnaire_e2e_db/);
assert.match(executableSql, /security definer/i);
...
assert.doesNotMatch(executableSql, /delete\s+from\s+public\.questionnaire_(responses|privacy_events)/i,
